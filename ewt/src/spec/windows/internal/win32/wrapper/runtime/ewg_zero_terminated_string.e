...
feature {NONE} -- Initialisation
	...
	make_unshared (a_item: POINTER) is
		...
		do
			create managed_data.make_unshared (a_item, EXTERNAL_STRING_.strlen_external (a_item))
		ensure
			item_not_default_pointer: item /= Default_pointer
	...
	make_shared (a_item: POINTER) is
			...
			-- not be freed as well.
		do
			create managed_data.make_shared (a_item, EXTERNAL_STRING_.strlen_external (a_item))
		ensure
			item_not_default_pointer: item /= Default_pointer
			item_set: item = a_item
			is_shared: is_shared
...
feature {ANY} -- Access
	...
	count: INTEGER is
		...
		do
			Result := EXTERNAL_STRING_.strlen_external (item)
		end
	...
	string: STRING is
		...
		do
			Result := EXTERNAL_STRING_.make_copy_from_c_zero_terminated_string (item)
		end
...
feature {ANY} -- Basic Operations
	...
	set_string (a_string: STRING) is
		...
		local
			source: POINTER
		do
			source := EXTERNAL_STRING_.string_to_pointer (a_string)
			source := EXTERNAL_MEMORY_.memcpy_external (item, source, a_string.count)
