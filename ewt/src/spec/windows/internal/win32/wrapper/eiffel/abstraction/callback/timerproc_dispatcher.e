...
feature {NONE}
	...
	make (a_callback: TIMERPROC_CALLBACK) is
		require
			a_callback_not_void: a_callback /= Void
		do
			callback := a_callback
			set_timerproc_entry_external (Current, $on_callback)
		end

feature {ANY}

	callback: TIMERPROC_CALLBACK

	c_dispatcher: POINTER is
		do
			Result := get_timerproc_stub_external
		end
