...
feature {NONE}
	...
	make (a_callback: WNDPROC_CALLBACK) is
		require
			a_callback_not_void: a_callback /= Void
		do
			callback := a_callback
			set_wndproc_entry_external (Current, $on_callback)
		end

feature {ANY}

	callback: WNDPROC_CALLBACK

	c_dispatcher: POINTER is
		do
			Result := get_wndproc_stub_external
		end
